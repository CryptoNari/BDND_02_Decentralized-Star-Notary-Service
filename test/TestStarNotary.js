...
it('can add the star name and star symbol properly', async() => {
    ...
    const tokenName = "SleddogStars";
    const tokenSymbol = "SDS";
...
it('lets 2 users exchange stars', async() => {
    ...
    let instance = await StarNotary.deployed();
    await instance.createStar('Star id: 6', starId1, {from: account1});
    await instance.createStar('Star id: 7', starId2, {from: account2});
    ...
    assert.equal(account2, await instance.ownerOf(starId1));
    assert.equal(account1, await instance.ownerOf(starId2));
});
...
it('lets a user transfer a star', async() => {
    ...
    let targetAccount = accounts[8];

    let instance = await StarNotary.deployed();

    await instance.createStar('Star id: 8', starId, {from: account});

    // Use the transferStar function implemented in the Smart Contract
    await instance.transferStar(targetAccount, starId, {from: account});
    // Verify the star owner changed.
    assert.equal(targetAccount, await instance.ownerOf(starId));
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    const starName = 'Star id: 9';

    let instance = await StarNotary.deployed();

    await instance.createStar('Star id: 9', starId, {from: account});
